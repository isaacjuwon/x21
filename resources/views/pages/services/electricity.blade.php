        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <section class="space-y-4">
                <div class="flex items-center justify-between">
                    <h3 class="text-sm font-bold uppercase tracking-wider text-zinc-500">1. Select Distributor</h3>
                    @if($this->operator_id)
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    @foreach ($this->operators as $operator)
                        ...
                        >
                            <div class="w-14 h-14 rounded-xl overflow-hidden mb-3 group-hover:scale-110 transition-transform">
            ...
            <section @class(['space-y-4 transition-all duration-500', 'opacity-50 pointer-events-none' => !$this->operator_id])>
                <div class="flex items-center justify-between">
                    <h3 class="text-sm font-bold uppercase tracking-wider text-zinc-500">2. Meter Type</h3>
                    @if($this->meter_type)
                    ...
                    @foreach(['prepaid', 'postpaid'] as $type)
                        <button 
                            ...
                            @class([
                                ...
                                'border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 bg-white dark:bg-zinc-800 hover:border-primary/30' => $this->meter_type != $type
                            ])
                        >
                            {{ $type }}
                        </button>
            ...
            <section @class(['space-y-4 transition-all duration-500', 'opacity-50 pointer-events-none' => !$this->meter_type])>
                <h3 class="text-sm font-bold uppercase tracking-wider text-zinc-500">3. Meter Number</h3>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none text-zinc-400">
                        <x-ui.icon name="hashtag" class="w-6 h-6" />
                    </div>
                    <input 
                        type="text" 
                        ...
                        @class([
                            ...
                            'border-zinc-200 dark:border-zinc-700 focus:border-primary' => !$errors->has('meter_number'),
                            'border-red-500 focus:border-red-500' => $errors->has('meter_number'),
                        ])
                    >
                </div>
                @error('meter_number')
            ...
            <section @class(['space-y-4 transition-all duration-500', 'opacity-50 pointer-events-none' => !$this->meter_number])>
                <h3 class="text-sm font-bold uppercase tracking-wider text-zinc-500">4. Amount</h3>
                <div class="relative">
                    ...
                    <input 
                        ...
                        placeholder="0.00" 
                        @class([
                            'w-full pl-12 pr-4 py-5 bg-white dark:bg-zinc-800 border-2 rounded-2xl focus:ring-4 focus:ring-primary/10 transition-all text-3xl font-black placeholder:text-zinc-200 dark:placeholder:text-zinc-700',
            ...
            <div class="sticky top-24 space-y-6">
                <div class="bg-white dark:bg-zinc-800 rounded-3xl shadow-xl overflow-hidden border border-zinc-100 dark:border-zinc-700">
                    <div class="p-6 bg-zinc-900 border-b border-zinc-800">
                        <h4 class="text-white font-bold uppercase tracking-widest text-xs">Payment Summary</h4>
                    </div>
                    ...
                    <div class="p-6 space-y-6">
                        @if($this->selectedoperator)
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <p class="text-xs text-zinc-500 font-bold uppercase tracking-wider">Distributor</p>
                                    <p class="font-black text-zinc-900 dark:text-white uppercase text-sm leading-tight">{{ $this->selectedoperator->name }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($this->amount > 0)
                            <div class="space-y-1">
                                <p class="text-xs text-zinc-500 font-bold uppercase tracking-wider">Total Charge</p>
                                <p class="font-black text-zinc-900 dark:text-white text-3xl leading-tight">{{ Number::currency($this->amount) }}</p>
                                <div class="flex items-center gap-2">
                                    <span class="px-2 py-0.5 bg-yellow-100 dark:bg-yellow-900/40 rounded text-[10px] font-bold text-yellow-700 dark:text-yellow-400 uppercase tracking-tighter">{{ $this->meter_type ?? 'Meter' }}</span>
                                    <span class="text-[10px] text-zinc-400 font-medium">Token instant delivery</span>
                                </div>
                            </div>
                        @else
                            <div class="py-12 text-center">
                                <div class="w-16 h-16 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl flex items-center justify-center mx-auto mb-4 border-2 border-dashed border-zinc-200 dark:border-zinc-700">
                                    <x-ui.icon name="light-bulb" class="w-8 h-8 text-zinc-300" />
                                </div>
                                <p class="text-xs text-zinc-400 font-medium max-w-[150px] mx-auto">Complete the form to see your payment summary</p>
                            </div>
                        @endif
                        ...
                        >
                            <span>Pay Bill</span>
                            <x-ui.icon name="paper-airplane" class="w-5 h-5 ml-2" />
                        </x-ui.button>
                ...
                <div class="bg-amber-500 rounded-3xl p-5 text-zinc-900 shadow-lg overflow-hidden relative group">
                    <div class="relative z-10">
                        <p class="text-[10px] font-bold uppercase tracking-widest text-amber-900/60 mb-1">Current Balance</p>
                        <p class="text-2xl font-black">{{ Number::currency(auth()->user()->wallet_balance) }}</p>
                    </div>
                    <x-ui.icon name="wallet" class="absolute -right-6 -bottom-6 w-28 h-28 text-amber-900/5 group-hover:scale-110 transition-transform" />
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-6">
            <div class="p-5 bg-zinc-50 dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-700">
                <div class="flex items-center gap-5 mb-8">
                    ...
                    <div>
                        <p class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Utility Provider</p>
                        <h4 class="text-2xl font-black text-zinc-900 dark:text-white leading-tight mb-1">{{ $this->selectedoperator?->name }}</h4>
                        <div class="flex gap-2">
                           <span class="px-2 py-1 rounded-lg bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[10px] font-black uppercase">{{ $this->meter_type }}</span>
                ...
                <div class="grid grid-cols-2 gap-y-6">
                    <div class="space-y-1">
                        <p class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Meter Number</p>
                        <p class="text-xl font-black tracking-widest text-zinc-900 dark:text-white">{{ $this->meter_number }}</p>
                    </div>
                    <div class="space-y-1 text-right">
                        <p class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Transaction Fee</p>
                        <p class="text-xl font-black text-emerald-600 uppercase transition-all">₹0.00</p>
                    </div>
                    <div class="col-span-2 pt-6 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                        <p class="text-sm font-black text-zinc-500 uppercase">Total to Pay</p>
